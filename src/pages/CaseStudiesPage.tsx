...
export function stripPersonalIntros(text: string): string {
  ...
  // Clean up weird punctuation and spaces
  t = t.replace(/[ \t]+([.,?!;:])/g, '$1')
...
function renderBlocks(text: string, keyOffset = 0, caseId?: string): ReactNode[] {
  ...
  segments.forEach((seg) => {
    ...
    const isCaptionLike = (b?: string) => {
      if (!b || b.includes('\n') || b.length > 90) return false
      if (/^(:::|#|-\s|\||[""]|http)/.test(b)) return false
      if (/^!\[Image\]\(/.test(b)) return false
      return true
    }
    let skipIndex = -1
    blocks.forEach((block: string, bi: number) => {
      if (bi === skipIndex) return
      const key = keyCount++
      ...
      if (block.startsWith(':::showcase')) {
        ...
        const isMobileApp = caseId && (caseId.includes('spaarks') || caseId.includes('stimuler') || caseId.includes('phonepe'))
        ...
        nodes.push(
          <div key={key} style={{
            ...
            {parsed.map((item, ii) => (
              isMobileApp ? (
                <div key={ii} className="showcase-row">
                  {/* Image Left */}
                  {item.imageUrl && (
                        ...
                        if (isBullet) {
                          return (
                            <div key={li} style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                              ...
                              <span style={{ fontSize: '1.05rem', color: 'var(--color-text-secondary)', lineHeight: 1.4 }}>{renderInline(trimmed.replace(/^[-*•]\s*/, ''))}</span>
                            </div>
                          )
                        }
                        return (
                          <p key={li} style={{ margin: 0, fontSize: '1.05rem', color: 'var(--color-text-secondary)', lineHeight: 1.5 }}>
                            {renderInline(line)}
                          </p>
                        )
                      })}
            ...
            {columns.map((colText, ci) => {
              ...
              return (
                <div key={ci} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
                  ...
                  {caption && (
                    <p style={{ margin: 0, fontSize: '0.75rem', fontWeight: 400, fontStyle: 'italic', lineHeight: 1.5, color: 'var(--color-text-secondary)' }}>
                      {caption}
                    </p>
                  )}
                </div>
...
export function renderContent(text: string, caseId?: string): ReactNode[] {
  ...
  if (swotStart >= 0 && threatsHeading >= 0) {
    ...
    return [
      ...renderBlocks(before, 0, caseId),
      <SWOTTable key="swot-table" text={swotText} />,
      ...renderBlocks(after, 10000, caseId),
    ]
  }

  return renderBlocks(text, 0, caseId)
}
          ...
          {selectedCaseId && activeCase && activeFolder && (
            ...
            >
              ...
              <div ref={scrollPanelRef} style={{ flex: 1, overflowY: isLocked ? 'hidden' : 'auto', position: 'relative', padding: 'var(--space-12) var(--space-12) var(--space-12)', filter: isLocked ? 'blur(6px)' : 'none', userSelect: isLocked ? 'none' : 'auto', pointerEvents: isLocked ? 'none' : 'auto' }}>
                ...
                {renderContent(stripPersonalIntros(activeCase.text), activeCase.id)}
              </div>
