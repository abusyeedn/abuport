...
class Title {
  ...
  constructor({ gl, plane, renderer, text, textColor = '#545050', font = `30px ${FONTS.primary}` }: TitleProps) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
...
class Media {
  ...
  update(scroll: { current: number; last: number }, direction: 'right' | 'left', delta: number = 0.016) {
    ...
    this.program.uniforms.uTime.value += delta * 2.5;
...
class App {
  ...
  update(time: number = 0) {
    if (this.isPaused) return;
    const delta = Math.min((time - this.lastTime) / 1000, 0.05); // seconds, capped at 50ms
    this.lastTime = time;
    this.scroll.current = lerp(this.scroll.current, this.scroll.target, this.scroll.ease);
    const direction = this.scroll.current > this.scroll.last ? 'right' : 'left';
    if (this.medias) {
      this.medias.forEach(media => media.update(this.scroll, direction, delta));
    }
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = (isSkipped: boolean) => {
      if (!app) return;
      if (isSkipped) {
        app.pause();
      } else {
    ...
    const onContentVisibilityChange = (e: Event & { skipped?: boolean }) => {
      if (e.skipped !== undefined) {
        handleVisibilityChange(e.skipped);
      }
    };

    const setupVisibilityObserver = () => {
      if (!element) return;
      element.addEventListener('contentvisibilityautostatechange', onContentVisibilityChange as EventListener);
    ...
    return () => {
      isMounted = false;
      if (element) {
        element.removeEventListener('contentvisibilityautostatechange', onContentVisibilityChange as EventListener);
      }
      if (observer) {
        observer.disconnect();
      }
