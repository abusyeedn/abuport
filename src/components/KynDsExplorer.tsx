...
function CopyableSwatch({ token }: { token: ColorToken }) {
  ...
  return (
    ...
    >
      <div style={{
        width: '48px', height: '32px', borderRadius: 'var(--radius-base)', background: token.value,
        border: '1px solid rgba(0,0,0,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center',
        marginBottom: 'var(--space-1)', position: 'relative'
      }}>
        {copied && <span style={{ fontSize: '0.5rem', fontWeight: 700, color: 'var(--color-text-primary)', background: 'rgba(255,255,255,0.85)', padding: '1px 3px', borderRadius: '3px', position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }}>Copied</span>}
      </div>
      <span style={{ fontSize: '0.58rem', color: 'var(--color-text-muted)', fontFamily: FONTS.mono }}>{shadeOnly}</span>
      <span style={{ fontSize: '0.55rem', color: 'var(--color-text-muted-light)', fontFamily: FONTS.mono }}>{token.value}</span>
    </button>
  )
}
...
export function KynDsComponentsBrowser() {
  ...
  return (
    <div style={{ marginTop: 'var(--space-2)' }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '14px' }}>
        {COMPONENTS.map((c) => (
          <button
            ...
            title={c.desc}
            style={{
              padding: '6px var(--space-3)', borderRadius: '999px', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer',
              border: `1px solid ${activeDoc === c.docId ? '#8b5cf6' : 'var(--color-border)'}`,
              background: activeDoc === c.docId ? '#8b5cf6' : '#ffffff',
              color: activeDoc === c.docId ? '#ffffff' : 'var(--color-text-tertiary)',
      ...
      <div style={{
        borderRadius: '14px', overflow: 'hidden', border: '1px solid var(--color-border)',
        boxShadow: '0 6px 24px rgba(0,0,0,0.06)', background: '#fff',
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', padding: 'var(--space-2) var(--space-3)', borderBottom: '1px solid var(--color-border)', background: 'var(--color-bg-secondary)' }}>
          ...
          <span style={{ marginLeft: 'var(--space-2)', fontSize: '0.7rem', color: 'var(--color-text-muted-light)', fontFamily: FONTS.mono }}>
...
export function KynDsSpacingRadius() {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-6)', marginTop: 'var(--space-2)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          {SPACING_SCALE.map((px) => (
            <div key={px} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <span style={{ width: '34px', flexShrink: 0, fontSize: '0.65rem', fontFamily: FONTS.mono, color: 'var(--color-text-muted-light)' }}>{px}px</span>
      ...
      <div>
        <h5 style={{ fontSize: '0.78rem', fontWeight: 700, color: 'var(--color-text-tertiary)', marginBottom: '10px' }}>Corner Radius</h5>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--space-3)' }}>
          {RADIUS_SCALE.map((r) => (
            <div key={r} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '5px' }}>
              ...
              <span style={{ fontSize: '0.6rem', fontFamily: FONTS.mono, color: 'var(--color-text-muted-light)' }}>{r}px</span>
            </div>
          ))}
        </div>
      </div>
...
export default function KynDsExplorer() {
  ...
  return (
    <div style={{ marginTop: 'var(--space-2)' }}>
      <h4 style={{ fontSize: '1rem', fontWeight: 700, margin: '0 0 6px', color: 'var(--color-text-primary)' }}>The Components — Live</h4>
      ...
      <div style={{ marginBottom: 'var(--space-8)' }}><KynDsComponentsBrowser /></div>

      <h4 style={{ fontSize: '1rem', fontWeight: 700, margin: '0 0 6px', color: 'var(--color-text-primary)' }}>Color Tokens</h4>
      ...
      <div style={{ marginBottom: 'var(--space-8)' }}><KynDsColorTokens /></div>

      <h4 style={{ fontSize: '1rem', fontWeight: 700, margin: '0 0 6px', color: 'var(--color-text-primary)' }}>Typography</h4>
      <p style={{ fontSize: '0.85rem', color: 'var(--color-text-muted)', marginBottom: '14px' }}>
        Manrope, 11 styles from 12px paragraphs to 48px display.
      </p>
      <div style={{ marginBottom: 'var(--space-8)' }}><KynDsTypeScale /></div>

      <h4 style={{ fontSize: '1rem', fontWeight: 700, margin: '0 0 6px', color: 'var(--color-text-primary)' }}>Spacing &amp; Radius</h4>
